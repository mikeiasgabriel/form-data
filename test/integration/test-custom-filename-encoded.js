...
var server = http.createServer(function (req, res) {
  ...
  form.parse(req, function (err, fields, files) {
    ...
    assert.strictEqual(
      files.custom_everything.name,
      options.filename,
      "Expects custom filename"
    );
    assert.strictEqual(
      files.custom_everything.type,
      options.contentType,
      "Expects custom content-type"
    );

    assert("custom_filename" in files);
    assert.strictEqual(
      files.custom_filename.name,
      options.filename,
      "Expects custom filename"
    );
    assert.strictEqual(
      files.custom_filename.type,
      mime.lookup(knownFile),
      "Expects original content-type"
    );

    assert("custom_filepath" in files);
    assert.strictEqual(
      files.custom_filepath.name,
      relativeFile.replace(/\\/g, "/"),
      "Expects custom filepath"
    );
    assert.strictEqual(
      files.custom_filepath.type,
      mime.lookup(knownFile),
      "Expects original content-type"
    );
    ...
    assert.strictEqual(
      files.unknown_with_filename.name,
      options.filename,
      "Expects custom filename"
    );
    assert.strictEqual(
      files.unknown_with_filename.type,
      mime.lookup(options.filename),
      "Expects filename-derived content-type"
    );

    assert("unknown_with_filename_as_object" in files);
    assert.strictEqual(
      files.unknown_with_filename_as_object.name,
      options.filename,
      "Expects custom filename"
    );
    assert.strictEqual(
      files.unknown_with_filename_as_object.type,
      mime.lookup(options.filename),
      "Expects filename-derived content-type"
    );
    ...
    assert.strictEqual(
      files.unknown_with_name_prop.name,
      options.filename,
      "Expects custom filename"
    );
    assert.strictEqual(
      files.unknown_with_name_prop.type,
      mime.lookup(options.filename),
      "Expects filename-derived content-type"
    );
    ...
    assert.strictEqual(
      files.unknown_everything.type,
      FormData.DEFAULT_CONTENT_TYPE,
      "Expects default content-type"
    );
...
server.listen(common.port, function () {
  ...
  form.append(
    "custom_filename",
    fs.createReadStream(knownFile),
    options.filename
  );
  // Filename only with unknown file
  form.append(
    "unknown_with_filename",
    fs.createReadStream(unknownFile),
    options.filename
  );
  // Filename only with unknown file
  form.append(
    "unknown_with_filename_as_object",
    fs.createReadStream(unknownFile),
    { filename: options.filename, encodeFilename: true }
  );
  // Filename with relative path
  form.append("custom_filepath", fs.createReadStream(knownFile), {
